    ...
    if (cardNumber.value.length >= 6) {
        PagSeguroDirectPayment.getBrand({
            ...
            success: function(res) {
                ...
                document.querySelector("input[name=brand_cartao]").value =
                    res.brand.name;
    ...
    el.addEventListener("submit", function(event) {
        ...
        if (paymentType === "creditcard") {
            PagSeguroDirectPayment.createCardToken({
                cardNumber: document.querySelector("input[name=numero_cartao]")
                    .value,
                ...
                expirationYear: document.querySelector("input[name=ano_cartao]")
                    .value,
                success: function(res) {
                    processPayment(
                        res.card.token,
...
function drawSelectInstallments(installments) {
    ...
    for (let l of installments) {
        select += `<option value="${l.quantity}|${l.installmentAmount}">${
            l.quantity
