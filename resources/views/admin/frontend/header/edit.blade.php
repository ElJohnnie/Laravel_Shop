    ...
    <div class="col-md-12">
        <div class="card">
            <div class="card-header" style="color:black;">
                <i class="fas fa-desktop"></i> Editar banner
            </div>
            <div class="card-body front-card">
                <form action="{{ route('admin.header.update', ['header' => $banner->id])}}" method="POST" enctype="multipart/form-data">
                    @csrf
                    @method("PUT")
                    <div class="form-row">
                        <div class="col-md-6">
                            <div class="form-group">
                                ...
                                <select name="product_id" id="product-banner-select" class="form-control">
                                    @foreach($products as $p)
                                        <option value="{{$p->id}}" @if($p->id == $banner->product_id) selected @endif>{{$p->name}}</option>
                                    @endforeach
                                </select>
                    ...
                    <div class="form-group">
                        <button class="btn btn-success" type="submit">Editar Banner</button>
                    </div>
                </form>
            </div>
        </div>
